...
gen_Lau <- genus %>% filter(geoplate %in% c("101","102","103","104","106","108","109","123","127","128","129"))
...
gen_Bal <- genus %>% filter(geoplate %in% c("302","303","304","305","307","308","309","311","313","315"))
...
gen_cont <- genus %>% filter(!geoplate %in% c("101","102","103","104","106","108","109","123","127","128","129",
                                              "302","303","304","305","307","308","309","311","313","315"))
...
# 
# gen_Lau <- genus_Ord %>% filter(geoplate %in% c("101","102","103","104","106","108","109","123","127","128","129"))
# gen_Lau_diff <- cbind(gen_Lau[,c("occurrence_no","accepted_name","max_ma","min_ma","geoplate")],
#                       "diff"=gen_Lau$max_ma-gen_Lau$min_ma)
# 
# gen_Bal <- genus_Ord %>% filter(geoplate %in% c("302","303","304","305","307","308","309","311","313","315"))
# gen_Bal_diff <- cbind(gen_Bal[,c("occurrence_no","accepted_name","max_ma","min_ma","geoplate")],
#                       "diff"=gen_Bal$max_ma-gen_Bal$min_ma)
# 
# gen_cont <- genus_Ord %>% filter(!geoplate %in% c("101","102","103","104","106","108","109","123","127","128","129",
#                                                   "302","303","304","305","307","308","309","311","313","315"))
# gen_cont_diff <- cbind(gen_cont[,c("occurrence_no","accepted_name","max_ma","min_ma","geoplate","ref_pubyr")],
#                        "diff"=gen_cont$max_ma-gen_cont$min_ma)
# 
...
##############################################################################
plot(Stagebase-0.2, BAL_Orig_rate[,27], type = "b", 
     pch=19,
     ylim = c(-0.1,0.5),
...
tscales.Ord(0.5, -0.05, -0.1)

lines(Stagebase-0.3, BAL_Orig_rate[,27], type = "b", pch=19, lwd=0.8)
lines(Stagebase[1]-1, BAL_Orig_rate[1,27]-0.02, type = "b", pch="B")
arrows(x0=Stagebase-0.3, y0=BAL_Orig_rate_CIl[,27], x1=Stagebase-0.3, y1=BAL_Orig_rate_CIu[,27], length=0.02, lwd = 0.8, angle = 90, code = 3)
lines(Stagebase, NAC_Orig_rate[,27], type = "b", lty = 2, pch=17,lwd=1.2)
lines(Stagebase[1]-1, NAC_Orig_rate[1,27]-0.02, type = "b", pch="L")
arrows(x0=Stagebase, y0=NAC_Orig_rate_CIl[,27], x1=Stagebase, y1=NAC_Orig_rate_CIu[,27], length=0.02, lwd = 1.2, lty=2, angle = 90, code = 3)
...
##############################################################################
## Extinction Rate
plot(Stagebase-0.2, BAL_Ext_rate[,27], type = "b",
     pch=19,
     ylim = c(-0.1,0.4),
...
tscales.Ord(0.4, -0.05, -0.1)

lines(Stagebase-0.3, BAL_Ext_rate[,27], type = "b", pch=19, lwd=0.8)
arrows(x0=Stagebase-0.3, y0=BAL_Ext_rate_CIl[,27], x1=Stagebase-0.3, y1=BAL_Ext_rate_CIu[,27], length=0.02, lwd = 0.8, angle = 90, code = 3)
lines(Stagebase, NAC_Ext_rate[,27], type = "b", lty = 2, lwd=1.2, pch=17)
arrows(x0=Stagebase, y0=NAC_Ext_rate_CIl[,27], x1=Stagebase, y1=NAC_Ext_rate_CIu[,27], length=0.02, lwd = 1.2,lty=2,  angle = 90, code = 3)

legend("topleft", legend="B", bty="n", cex = 1.25)

axis(2, col = 'grey75', line = -0.2, at = seq(0, 0.4, 0.1))
...
##############################################################################
## net diversification 
plot(Stagebase-0.2, div_BAL[,27]-1, type = "b", 
     ylim = c(-2,8),
     xlim = rev(c(444.18,485.4)),
     axes = F,
     xlab = "",
...
lines(Stagebase-0.2, div_BAL[,27]-1, type = "b", lwd=0.8, pch=19)
arrows(x0=Stagebase-0.2, y0=divl_BAL[,27]-1, x1=Stagebase-0.2, y1=divu_BAL[,27]-1, length=0.02, lwd = 0.8, angle = 90, code = 3)
lines(Stagebase, div_NAC[,27]-1, type = "b", lty = 2, pch=17, lwd=0.8)
arrows(x0=Stagebase, y0=divl_NAC[,27]-1, x1=Stagebase, y1=divu_NAC[,27]-1, length=0.02, lwd = 1.2, lty=2,  angle = 90, code = 3)
...
##############################################################################
## Sampling rate
plot(Stagemidpoints-0.1, rate_p_BAL[,27], type = 'b', 
     xlim = rev(c(444.18,485.4)),
     ylim = c(-0.02,0.8),
     axes = F,
     xlab = "", ylab = "")

tscales.Ord(0.8, 0.01, -0.02)

lines(Stagemidpoints-0.1, rate_p_BAL[,27], type = "b", pch=19,lwd=0.8)
arrows(x0=Stagemidpoints-0.1, y0=ratel_p_BAL[,27], x1=Stagemidpoints-0.1, y1=rateu_p_BAL[,27], length=0.02, lwd = 0.8, angle = 90, code = 3)
lines(Stagemidpoints+0.1, rate_p_LAU[,27], type = "b" , lty = 2, lwd=1.2, pch=17)
arrows(x0=Stagemidpoints+0.1, y0=ratel_p_LAU[,27], x1=Stagemidpoints+0.1, y1=rateu_p_LAU[,27], length=0.02, lwd = 1.2, lty=2, angle = 90, code = 3)
...
################################################################################################
##### Figure S3 ## replicate of 100 runs #######################################################
...
plot(Stagemidpoints, N_B[3:9,27],
     type = "b", 
     ylim = c(-50,730),
     xlim = rev(c(444.18,485.4)),
     axes = F,
...
tscales.Ord(730, 0, -60)

lines(Stagemidpoints-0.2, N_B[3:9,27], type = "b", pch=19, lwd=0.8)
lines(Stagemidpoints[1]-1, N_B[3,27]-25, type = "p", pch="B")
arrows(x0=Stagemidpoints-0.2, y0=N_B_lcI[3:9,27], x1=Stagemidpoints-0.2, y1=N_B_ucI[3:9,27],
       length=0.02, lwd = 0.8, angle = 90, code = 3)
lines(Stagemidpoints+0.2, N_L[3:9,27], type = "b", pch=17, lwd=1.2, lty=2)
lines(Stagemidpoints[1]+1, N_L[3,27]+20, type = "p", pch="L")
arrows(x0=Stagemidpoints+0.2, y0=N_L_lcI[3:9,27], x1=Stagemidpoints+0.2, y1=N_L_ucI[3:9,27],
       length=0.02, lwd = 1.2, angle = 90, lty=2, code = 3)


axis(1, col = 'grey75', line = 0.5, at = seq(445,485,10) )
axis(2, col = 'grey75', line = -0.2, at = seq(0, 750, 150))
...
plot(Stagemidpoints, mean.N.B[3:9],
     ...
     xlab = "",
     ylab = "")
tscales.Ord(900, 0, -60)


for (i in 1:100){
  lines(Stagemidpoints, N_B_lcI[3:9,i], col = "grey70")}
for (i in 1:100){
  lines(Stagemidpoints, N_B_ucI[3:9,i], col = "grey70")}

for (i in 1:100){
  lines(Stagemidpoints, N_L_lcI[3:9,i], col = "grey55")}
for (i in 1:100){
  lines(Stagemidpoints, N_L_ucI[3:9,i], col = "grey55")}
...
axis(1, col = 'grey75', line = 0.5, at = seq(445,485,10) )
axis(2, col = 'grey75', line = -0.2, at = seq(0, 900, 150))
