...
## commands are as in the onoff_org file
## except that the filter further down is set on the last occurrence instead of the first one
...
## HERE we check where the genera have their last occurrence
    last  <-   setNames(data.frame(t(apply(rev(test[1:11]), 1, function(x) {
      ind <- which(!is.na((x)))[1]
      c(ind, x[ind])
    }))),
    c("Stage", "last"))
    ...
    test_last <- cbind(test, "last"=last$last)

    extract_inp_last <- filter(test_last, last=="on" | last=="off")
    
    extract_inp_last$environment <- ifelse(extract_inp_last$last=="on", "onshore", "offshore")
    inp_mig <- ifelse(!is.na(extract_inp_last[1:11]),1,0)
    data_onoff <- cbind.data.frame(inp_mig, environment = extract_inp_last$environment)
    ...
    inp_mig <- cbind(inp_mig, environment = extract_inp_last$environment, ";") ### FOR FINAL INPUT DATASET
...
##############################################################################
##############################################################################
## extracting parameter estimates from results above
...
## tranformation of probabilities into rates
...
####################################
# ## run env.model
# ## results of several runs don't differ from each other (therefore was loop removed)
# 
# 
...
#                                                   "marginal marine indet.",
#                                                   #        "open shallow subtidal","shallow subtidal indet.","estuary/bay",
#                                                   "carbonate indet.","peritidal",
#                                                   "sand shoal", "shoreface"))
# #        , "slope"))
...
# #           "deep-water indet."))
# #           "deep subtidal indet.",  "deep subtidal ramp", "deep subtidal shelf"))
...
# 
# # #
# #test how many of the genera are the same
# overlap <- as.factor(Reduce(intersect, list(cast_on$accepted_name, cast_off$accepted_name)))
# overlap_onelse <- as.factor(Reduce(intersect,list(cast_on$accepted_name, cast_else$accepted_name)))
# overlap_offelse <-   as.factor(Reduce(intersect, list(cast_off$accepted_name, cast_else$accepted_name)))
# 
...
# 
# ## deal with on and else overlaps
# onelse_OL <- cast_on %>% filter(!accepted_name %in% overlap)
# onelse_OL <- onelse_OL %>% filter(accepted_name %in% overlap_onelse)
# elseon_OL <- cast_else %>% filter(!accepted_name %in% overlap)
# elseon_OL <- elseon_OL %>% filter(accepted_name %in% overlap_onelse)  
...
# 
# ## deal with off and else overlaps
# offelse_OL <- cast_off %>% filter(!accepted_name %in% overlap)
# offelse_OL <- offelse_OL %>% filter(accepted_name %in% overlap_offelse)
# elseoff_OL <- cast_else %>% filter(!accepted_name %in% overlap)
# elseoff_OL <- elseoff_OL %>% filter(accepted_name %in% overlap_offelse)  
...
# 
# ## for extra migration
# test_mig <- rbind.data.frame(inp_onoff,inp_onelse,inp_offelse) ### EVERYTHING FOR MIG-DATA GOES IN HERE
# test_mig[test_mig == 3] <- "on+off"
# test_mig[test_mig == 2] <- "off"
# test_mig[test_mig == 1] <- "on"
# test_mig[test_mig == 11] <- "on+else"
...
# test <- as.data.frame(test_mig)
# 
# last  <-   setNames(data.frame(t(apply(rev(test[1:7]), 1, function(x) {
#   ind <- which(!is.na((x)))[1]
#   c(ind, x[ind])
# }))),
# c("Stage", "last"))
...
# extract_inp_last <- filter(test_last, last=="on" | last=="off")
# 
# ## depending on how I assign groups I could define onshore-migration and offshore-migration here! last=="on", "onmig", "offmig") 
# extract_inp_last$environment <- ifelse(extract_inp_last$last=="on", "onshore", "offshore")
# inp_mig <- ifelse(!is.na(extract_inp_last[1:7]),1,0)
...
# ##############################################################################
# ##############################################################################
# ###extracts from environment model###########################################
...
text(Stagebase[1]-5, off_Orig_rate[1,27]-0.15, labels="off",lwd=1.2)
...
## overlapping data
phyla_mig <- rbind.data.frame(phyla_offelse,phyla_onelse,phyla_onoff) ### phyla-datasets from above (env.model)
...
phyla_last <- cbind(phyla_mig, "last"=last$last)


phyla_last <- filter(phyla_last, last=="on" | last=="off")
phyla_last$environment <- ifelse(phyla_last$last=="on", "onshore", "offshore")


extract_inp_last <- cbind.data.frame("accepted_name"=phyla_last$accepted_name,"environment"=phyla_last$environment)

phyla_migon <- filter(extract_inp_last, environment=="onshore")
phyla_migon <- genus_phyla %>% filter(accepted_name %in% phyla_migon$accepted_name)

phyla_migoff <- filter(extract_inp_last, environment=="offshore")
...
onoffelse_phyla <- full_join(nend_on,
                             nend_off,
                             by = "Group.1")

colnames(onoffelse_phyla)[1] <- "Phylum"
colnames(onoffelse_phyla)[2] <- "onshore"
colnames(onoffelse_phyla)[3] <- "offshore"



onoffelse_phyla[,2] <- onoffelse_phyla[,2]*100/colSums(onoffelse_phyla[2], na.rm = T)
onoffelse_phyla[,3] <- onoffelse_phyla[,3]*100/colSums(onoffelse_phyla[3], na.rm = T)


end_on <- data.frame(onoffelse_phyla$Phylum,onoffelse_phyla$onshore)
end_on <- arrange(end_on,desc(end_on[,2]))#, n = 10)
end_on <- na.omit(end_on)
end_on$group <- ifelse(end_on[,2]>1.5, 1, 2)
end_on1 <- end_on[end_on$group==1,]


end_on1$onoffelse_phyla.Phylum <- factor(end_on1$onoffelse_phyla.Phylum,
                                         levels = c(levels(end_on1$onoffelse_phyla.Phylum),"Others"))

end_on1[nrow(end_on1)+1,1] <- c("Others")
end_on1[nrow(end_on1),2] <- c(100-colSums(end_on1[2], na.rm = T))
endemics_on <- end_on1[,1:2]


end_off <- data.frame(onoffelse_phyla$Phylum,onoffelse_phyla$offshore)
end_off <- arrange(end_off,desc(end_off[,2]))#, n = 10)
end_off <- na.omit(end_off)
end_off$group <- ifelse(end_off[,2]>1.5, 1, 2)
end_off1 <- end_off[end_off$group==1,]


end_off1$onoffelse_phyla.Phylum <- factor(end_off1$onoffelse_phyla.Phylum,
                                          levels = c(levels(end_off1$onoffelse_phyla.Phylum),"Others"))

end_off1[nrow(end_off1)+1,1] <- c("Others")
end_off1[nrow(end_off1),2] <- c(100-colSums(end_off1[2], na.rm = T))
endemics_off <- end_off1[,1:2]




df_onoffmig <- rbind.data.frame(cbind.data.frame(
  "Phylum"=endemics_on[,1],
  "percent"=endemics_on[,2],
  "id"="onshore"),
  cbind.data.frame(
    "Phylum"=endemics_off[,1],
    "percent"=endemics_off[,2],
    "id"="offshore"))#,

ggplot(df_onoffmig, aes(x="", y=percent, fill=Phylum)) + 
